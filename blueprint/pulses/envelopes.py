...
def _net_zero_envelope(
    t: float | Array,
    /,
    pad_times: float | Array,
    hold_times: float | Array,
    transition_times: float | Array,
) -> Array:
    first_hold_times = (t >= pad_times) & (t < pad_times + hold_times)
    start_times = pad_times + hold_times + transition_times
    second_hold_times = (t >= start_times) & (t < start_times + hold_times)

    first_envelope = jnp.where(first_hold_times, 1, 0)
    second_envelope = jnp.where(second_hold_times, -1, 0)
    envelope = first_envelope + second_envelope
    return envelope


def net_zero_envelope(
    t: float | Array,
    /,
    ...
    """Returns a Net zero pulse envelopes at time(s) `t` where the amplitude is `+1.0`
    for half of `hold_times`, then `0.0` for `transition_times` and finally `-1.0` for
    the other half of `hold_times`.
    Potentially batching over the pulse parameters
    (`pad_times`, `hold_times`, `transition_times`).
    ...
    Returns:
        _(array of shape (t?, pad_times?, hold_times?))_ Pulse envelopes.
    """
    t, pad_times, hold_times, transition_times = format_pulse_params(
        [t, pad_times, hold_times, transition_times]
    )
    return jnp.squeeze(
        _net_zero_envelope(
            t,
